...
tags: ["query:agent"]
score: 7.0
evidence: 通信延迟下的多智能体子模最大化协作
tldr: 针对动态环境下存在通信延迟的多智能体次模最大化问题，本文提出分布式在线贪婪（DOG）算法。该算法结合对抗性多臂老虎机学习与延迟反馈机制，实现了任意网络拓扑下的同步决策。研究揭示了协调性能与收敛时间随通信延迟变化的权衡关系，填补了全集中式与局部去中心化方法之间的空白。
source: arxiv
selection_source: fresh_fetch
motivation: 现有的在线次模最大化方法在处理通信延迟时，要么因顺序通信导致高延迟，要么因仅限一跳邻居协调而限制了性能。
method: 提出DOG算法，利用带延迟反馈的对抗性老虎机学习工具，支持在任意网络拓扑中进行多智能体同步决策。
result: 理论分析给出了算法相对于最优解的近似性能，并量化了去中心化成本与网络结构及通信延迟之间的权衡关系。
conclusion: DOG算法成功衔接了全集中式与局部去中心化协调方法，为延迟环境下的分布式信息采集任务提供了高效方案。
---

## 摘要
我们针对通信延迟下的多智能体次模最大化问题，提出了一种分布式在线算法。我们的研究动机源于未知动态环境中的未来分布式信息采集任务，在这些任务中，效用函数自然地表现出边际收益递减特性，即次模性。现有的在线次模最大化方法要么依赖于顺序多跳通信，导致不可接受的延迟和严格的连通性假设；要么将每个智能体的协调限制在其一跳邻域内，从而限制了协调性能。为解决这一问题，我们提出了分布式在线贪婪（DOG）算法，该算法结合了带延迟反馈的对抗性多臂老虎机学习工具，以实现在任意网络拓扑结构下的同步决策。我们给出了 DOG 算法相对于最优解的近似性能，并将由于去中心化导致的次优成本刻画为网络结构的函数。我们的分析进一步揭示了协调性能与收敛时间之间的权衡，这种权衡由通信延迟的大小决定。通过这种权衡，DOG 算法涵盖了从最先进的全中心化在线协调方法 [1] 到全去中心化一跳协调方法 [2] 的整个范畴。
